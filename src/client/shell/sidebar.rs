...
pub(crate) fn render_sidebar(
    ...
    let layout = super::sidebar_status::expanded_sidebar_layout(
        area,
        state.sidebar_section_split,
        state.sidebar_status_height,
        state.sidebar_status_active && config.sidebar_status.enabled(),
    );
    let workspace_area = layout.workspace;
    let detail_area = layout.detail;
    hits.sidebar_section_divider = layout.section_divider;
    hits.sidebar_status = layout.status;
    hits.sidebar_status_divider = layout.status_divider;
    super::sidebar_status::render_divider(
        buffer,
        layout.status_divider,
        if state.sidebar_status_focused {
            palette.accent
        } else {
            palette.surface_dim
        },
...
mod tests {
    ...
    #[test]
    fn disabled_status_keeps_the_existing_expanded_sidebar_geometry() {
        let area = Rect::new(4, 2, 30, 20);
        let split = 0.45;
        let expected = crate::ui::expanded_sidebar_sections(area, split);
        let content = Rect::new(area.x, area.y, area.width.saturating_sub(1), area.height);
        let status_height = super::super::sidebar_status::block_height(8, content.height, false);
        ...
        assert_eq!(
            crate::ui::expanded_sidebar_sections(sections, split),
            expected
        );
        assert_eq!(
            crate::ui::sidebar_section_divider_rect(sections, split),
