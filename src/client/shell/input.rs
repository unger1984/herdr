        ...
        for event in events {
            ...
            match event {
                ...
                RawInputEvent::Text(text) => {
                    ...
                    if self.sidebar_status_focused {
                        if let Some(status) = self.sidebar_status_runtime.as_ref() {
                            let _ = status.runtime.try_send_bytes(bytes::Bytes::from(text));
                        } else {
                ...
                RawInputEvent::Paste(text) => {
                    ...
                    if self.sidebar_status_focused {
                        if let Some(status) = self.sidebar_status_runtime.as_ref() {
                            let _ = status.runtime.try_send_paste(text);
                        } else {
                            self.sidebar_status_focused = false;
                        }
                        outcome.repaint = true;
            ...
            ClientShellMode::Terminal => {
                ...
                if self.sidebar_status_focused {
                    if key.code == KeyCode::Esc && key.modifiers.is_empty() {
                        self.sidebar_status_focused = false;
                        outcome.repaint = true;
                    } else if !super::sidebar_status::send_key(
                        self.sidebar_status_runtime.as_ref(),
                        key.clone(),
                    ) {
                        self.sidebar_status_focused = false;
                    }
