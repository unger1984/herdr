...
pub(crate) struct ClientShellConfig {
    ...
    pub(super) sidebar_status: crate::config::SidebarStatusConfig,
...
pub(super) struct ShellHitMap {
    ...
    pub(super) sidebar_status: Rect,
    pub(super) sidebar_status_divider: Rect,
...
pub(crate) struct ClientShellState {
    ...
    pub(super) sidebar_status_height: u16,
    pub(super) sidebar_status_height_manual: bool,
    pub(super) sidebar_status_focused: bool,
    pub(super) sidebar_status_runtime: Option<super::sidebar_status::SidebarStatusRuntime>,
    pub(super) sidebar_status_events: tokio::sync::mpsc::Receiver<crate::events::AppEvent>,
    pub(super) sidebar_status_event_tx: tokio::sync::mpsc::Sender<crate::events::AppEvent>,
    pub(super) sidebar_status_render_dirty: std::sync::Arc<crate::render_signal::RenderSignal>,
    pub(super) sidebar_status_render_notify: std::sync::Arc<tokio::sync::Notify>,
...
impl Drop for ClientShellState {
    fn drop(&mut self) {
...
impl ClientShellState {
    pub(super) fn sync_sidebar_status_runtime(&mut self) {
        let config = self.config.sidebar_status.clone();
        ...
        match crate::terminal::TerminalRuntime::spawn_argv_command(
            pane_id,
            self.sidebar_status_height.max(1),
            self.sidebar_width.saturating_sub(1).max(1),
            cwd,
            &argv,
            &launch_env,
            ...
            self.sidebar_status_event_tx.clone(),
            self.sidebar_status_render_notify.clone(),
            self.sidebar_status_render_dirty.clone(),
        ) {
            Ok(runtime) => {
                self.sidebar_status_runtime = Some(super::sidebar_status::SidebarStatusRuntime {
                    pane_id,
            ...
            Err(error) => tracing::warn!(%error, "failed to spawn sidebar status command"),
        }
    }

    pub(super) fn shutdown_sidebar_status_runtime(&mut self) {
        if let Some(status) = self.sidebar_status_runtime.take() {
            status.runtime.shutdown();
        }
        self.sidebar_status_focused = false;
    }

    pub(crate) fn tick_sidebar_status(&mut self) -> bool {
        let mut repaint = self.sidebar_status_render_dirty.is_pending();
        if repaint {
            let _ = self.sidebar_status_render_dirty.take();
        }
        while let Ok(event) = self.sidebar_status_events.try_recv() {
            if let crate::events::AppEvent::PaneDied { pane_id } = event {
                if self
                    .sidebar_status_runtime
                    .as_ref()
                    .is_some_and(|status| status.pane_id == pane_id)
                {
                    if let Some(status) = self.sidebar_status_runtime.as_mut() {
                        status.exited = true;
                    }
                    self.sidebar_status_focused = false;
                    repaint = true;
    ...
    pub(crate) fn new(mut config: ClientShellConfig) -> Self {
        ...
        let sidebar_section_split = preferences
            ...
            .unwrap_or(0.5);
        let sidebar_status_height = preferences
            .sidebar_status_height
            .unwrap_or(config.sidebar_status.height);
        let (sidebar_status_event_tx, sidebar_status_events) = tokio::sync::mpsc::channel(8);
        ...
        Self {
            ...
            sidebar_status_height,
            sidebar_status_height_manual: preferences.sidebar_status_height.is_some(),
            sidebar_status_focused: false,
            sidebar_status_runtime: None,
            sidebar_status_events,
            sidebar_status_event_tx,
            sidebar_status_render_dirty: std::sync::Arc::new(
                crate::render_signal::RenderSignal::new(),
            ),
            sidebar_status_render_notify: std::sync::Arc::new(tokio::sync::Notify::new()),
...
mod tests {
    ...
    #[tokio::test]
    async fn status_runtime_death_releases_local_focus_and_keeps_the_runtime_for_restart() {
        let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
        let pane_id = crate::layout::PaneId::alloc();
        let (runtime, _) = crate::terminal::TerminalRuntime::test_with_channel(12, 3);
        state.sidebar_status_runtime = Some(super::super::sidebar_status::SidebarStatusRuntime {
            pane_id,
            command: vec!["status".into()],
            runtime,
            exited: false,
        });
        state.sidebar_status_focused = true;
        state
            .sidebar_status_event_tx
            .try_send(crate::events::AppEvent::PaneDied { pane_id })
            .expect("queue status exit");

        assert!(state.tick_sidebar_status());
        assert!(!state.sidebar_status_focused);
        assert!(state
            .sidebar_status_runtime
            .as_ref()
    ...
    #[test]
    fn status_focus_releases_for_escape_and_workspace_navigation() {
        let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
        state.sidebar_status_focused = true;
