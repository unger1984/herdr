...
fn install_sidebar_status_runtime(state: &mut ClientShellState) {
    let pane_id = crate::layout::PaneId::alloc();
    let (runtime, _) = crate::terminal::TerminalRuntime::test_with_channel(24, 4);
    state.config.sidebar_status = crate::config::SidebarStatusConfig {
        command: vec!["status".into()],
        height: 5,
    };
    state.sidebar_status_height = 5;
...
#[tokio::test]
async fn expanded_multi_endpoint_sidebar_reserves_and_hits_local_status_block() {
    let (mut state, _) = state_with_remote();
    install_sidebar_status_runtime(&mut state);

    state.compose(100, 30).expect("compose federated sidebar");

    assert!(!state.hits.sidebar_status_divider.is_empty());
    assert!(!state.hits.sidebar_status.is_empty());
    assert_eq!(state.hits.sidebar_status.height, 4);
    assert!(state.hits.agent_body.bottom() <= state.hits.sidebar_status_divider.y);
}

#[tokio::test]
async fn unavailable_shell_composes_local_status_block() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    install_sidebar_status_runtime(&mut state);

    state.compose(80, 20).expect("compose unavailable shell");
