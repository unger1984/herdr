...
impl ClientShellState {
    fn render_sidebar_status_runtime(&self, buffer: &mut Buffer) {
        if self.hits.sidebar_status.is_empty() {
            return;
        }
        let Some(status) = self.sidebar_status_runtime.as_ref() else {
            return;
        };
        let area = self.hits.sidebar_status;
        status.runtime.resize(area.height, area.width, 0, 0);
        let mut terminal =
            ratatui::Terminal::new(ratatui::backend::TestBackend::new(area.width, area.height))
                .ok();
        if let Some(terminal) = terminal.as_mut() {
            let sidebar_bg = self.config.palette.sidebar_bg;
            let _ = terminal.draw(|frame| {
                let frame_area = frame.area();
                frame
                    .buffer_mut()
                    .set_style(frame_area, Style::default().bg(sidebar_bg));
                status
                    .runtime
                    .render(frame, Rect::new(0, 0, area.width, area.height), false);
            });
            let rendered = terminal.backend().buffer();
            for y in 0..area.height {
                for x in 0..area.width {
                    buffer[(area.x + x, area.y + y)] = rendered[(x, y)].clone();
                }
            }
        }
    }
    ...
    fn compose_unavailable(&mut self, cols: u16, rows: u16) -> FrameData {
        ...
        super::endpoint_sidebar::render_expanded(
            ...
            &mut render::ShellRenderState {
                ...
                sidebar_status_height: self.sidebar_status_height,
                sidebar_status_active: self.sidebar_status_runtime.is_some(),
                sidebar_status_focused: self.sidebar_status_focused,
        ...
        self.render_sidebar_status_runtime(&mut buffer);
