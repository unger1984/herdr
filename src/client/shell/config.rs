...
impl ClientShellState {
    ...
    pub(super) fn persist_chrome_preferences(&mut self, outcome: &mut ClientShellInput) {
        ...
        let preferences = preferences::ClientChromePreferences {
            ...
            sidebar_status_height: self
                .sidebar_status_height_manual
                .then_some(self.sidebar_status_height),
    ...
    pub(crate) fn reload_client_config(&mut self) {
        match crate::config::load_live_config() {
            Ok(loaded) => {
                ...
                if !self.sidebar_status_height_manual {
                    self.sidebar_status_height = self.config.sidebar_status.height;
                }
    ...
    pub(super) fn apply_live_config(
        ...
        if !invalid_section("ui") {
            if let Some(diagnostic) = config.invalid_sidebar_bounds_diagnostic() {
                ...
                self.sidebar_status = ui.sidebar.status.clone();
...
mod tests {
    ...
    fn live_reload_applies_client_owned_sections() {
        ...
        next.ui.sidebar.status = crate::config::SidebarStatusConfig {
        ...
        assert_eq!(shell.sidebar_status.command, ["limits"]);
        assert_eq!(shell.sidebar_status.height, 6);
        assert_eq!(
