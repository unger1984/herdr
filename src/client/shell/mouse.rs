...
impl ClientShellState {
    ...
    fn set_sidebar_status_height_from_row(&mut self, row: u16, outcome: &mut ClientShellInput) {
        ...
        let height = sidebar
            ...
            .saturating_sub(row)
            .max(1)
            .min(sidebar.height.saturating_sub(4));
        if self.sidebar_status_height != height {
            self.sidebar_status_height = height;
            self.sidebar_status_height_manual = true;
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        ...
        if mouse.kind == MouseEventKind::Drag(MouseButton::Left) {
            match self.chrome_drag.as_ref() {
                ...
                Some(ClientChromeDrag::SidebarStatus) => {
                    self.set_sidebar_status_height_from_row(mouse.row, outcome);
                    return;
                }
        ...
        if mouse.kind == MouseEventKind::Up(MouseButton::Left) {
            if let Some(drag) = self.chrome_drag.take() {
                ...
                match drag {
                    ...
                    | ClientChromeDrag::SidebarStatus => {
                        self.persist_chrome_preferences(outcome);
                    }
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if super::contains(self.hits.sidebar_status_divider, point) {
                    self.chrome_drag = Some(ClientChromeDrag::SidebarStatus);
                    self.set_sidebar_status_height_from_row(mouse.row, outcome);
                    return;
                }
                if super::contains(self.hits.sidebar_status, point) {
                    if self
                    ...
                    if self.sidebar_status_runtime.is_none() {
                        self.sync_sidebar_status_runtime();
                    }
                    self.sidebar_status_focused = self.sidebar_status_runtime.is_some();
                    outcome.repaint = true;
                    return;
                }
...
mod tests {
    ...
    #[test]
    fn outside_sidebar_status_click_releases_focus() {
